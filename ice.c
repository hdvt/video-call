...
gint janus_ice_handle_attach_plugin(void *core_session, janus_ice_handle *handle, janus_plugin *plugin, const char *token)
{
	if (core_session == NULL)
...
static void janus_ice_cb_nice_recv(NiceAgent *agent, guint stream_id, guint component_id, guint len, gchar *buf, gpointer ice)
{
	...
	janus_session *session = (janus_session *)handle->session;
	if (!component->dtls)
	{ /* Still waiting for the DTLS stack */
	...
	if (janus_is_rtp(buf, len))
	{
		...
		else
		{
			...
			video = ((stream->video_ssrc_peer[0] == packet_ssrc || stream->video_ssrc_peer_rtx[0] == packet_ssrc || stream->video_ssrc_peer[1] == packet_ssrc || stream->video_ssrc_peer_rtx[1] == packet_ssrc || stream->video_ssrc_peer[2] == packet_ssrc || stream->video_ssrc_peer_rtx[2] == packet_ssrc) ? 1 : 0);
			...
			else
			{
				...
				if (video)
				{
					...
					int nstate = stream->rtx_nacked[vindex] ? GPOINTER_TO_INT(g_hash_table_lookup(stream->rtx_nacked[vindex], GUINT_TO_POINTER(seqno))) : 0;
					if (nstate == 1)
					{
...
void janus_ice_setup_remote_candidates(janus_ice_handle *handle, guint stream_id, guint component_id)
{
	...
	{
		if (!janus_flags_is_set(&handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_TRICKLE) || janus_flags_is_set(&handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_ALL_TRICKLES))
		{
...
static gboolean janus_ice_outgoing_traffic_handle(janus_ice_handle *handle, janus_ice_queued_packet *pkt)
{
	...
	if (pkt->control)
	{
		...
		else
		{
			...
			int res = janus_is_webrtc_encryption_enabled() ? srtp_protect_rtcp(component->dtls->srtp_out, pkt->data, &protected) : srtp_err_status_ok;
			if (res != srtp_err_status_ok)
			{
	...
	else
	{
		...
		if (pkt->type == JANUS_ICE_PACKET_AUDIO || pkt->type == JANUS_ICE_PACKET_VIDEO)
		{
			...
			else
			{
				...
				/* Encrypt SRTP */
				int protected = pkt->length;
				int res = janus_is_webrtc_encryption_enabled() ? srtp_protect(component->dtls->srtp_out, pkt->data, &protected) : srtp_err_status_ok;
				if (res != srtp_err_status_ok)
				{
					/* We don't spam the logs for every SRTP error: just take note of this, and print a summary later */
					handle->srtp_errors_count++;
				...
				else
				{
					...
					if (sent > 0)
					{
						...
						if (pkt->type == JANUS_ICE_PACKET_AUDIO)
						{
							...
							rtcp_context *rtcp_ctx = stream->audio_rtcp_ctx;
							int pt = header->type;
							uint32_t clock_rate = stream->clock_rates ? GPOINTER_TO_UINT(g_hash_table_lookup(stream->clock_rates, GINT_TO_POINTER(pt))) : 48000;
...
void janus_ice_relay_rtp(janus_ice_handle *handle, janus_plugin_rtp *packet)
{
	if (!handle || !handle->stream || handle->queued_packets == NULL || packet == NULL || packet->buffer == NULL ||
		...
		return;
	if ((!packet->video && !janus_flags_is_set(&handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_HAS_AUDIO)) || (packet->video && !janus_flags_is_set(&handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_HAS_VIDEO)))
		return;
