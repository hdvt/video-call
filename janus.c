		...
		if (result->type == JANUS_PLUGIN_OK)
		{
			...
			if (result->content == NULL || !json_is_object(result->content))
			{
				...
				ret = janus_process_error(request, session_id, transaction_text, JANUS_ERROR_PLUGIN_MESSAGE,
										  result->content == NULL ? "Plugin didn't provide any content for this synchronous response" : "Plugin returned an invalid JSON response");
...
static int janus_request_allow_token(janus_request *request, guint64 session_id, const gchar *transaction_text, gboolean allow, gboolean add)
{
	...
	if (add)
	{
		...
		if (!mysql_handler_insert(query))
		{
			janus_auth_remove_token(token_value);
			ret = janus_process_error(request, session_id, transaction_text, JANUS_ERROR_UNKNOWN, "Error adding token");
			goto jsondone;
		}
	}
...
int janus_process_incoming_admin_request(janus_request *request)
{
	...
	else
	{
		...
		json_object_set_new(info, "loop-running", (handle->mainloop != NULL && g_main_loop_is_running(handle->mainloop)) ? json_true() : json_false());
	...
	if (sessions && g_hash_table_size(sessions) > 0)
	{
		...
		while (g_hash_table_iter_next(&iter, NULL, &value))
		{
			...
			if (session->source && session->source->instance == transport)
			{
				...
				if (reclaim_session_timeout < 1)
				{ /* Reclaim session timeouts are disabled */
...
int janus_plugin_push_event(janus_plugin_session *plugin_session, janus_plugin *plugin, const char *transaction, json_t *message, json_t *jsep)
{
	...
	if (sdp_type != NULL && sdp != NULL)
	{
		...
		if (merged_jsep == NULL)
		{
			if (ice_handle == NULL || janus_flags_is_set(&ice_handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_STOP) || janus_flags_is_set(&ice_handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_ALERT))
			{
	...
	if ((restart || janus_flags_is_set(&ice_handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_RESEND_TRICKLES)) && janus_ice_is_full_trickle_enabled())
	{
...
void janus_plugin_relay_rtp(janus_plugin_session *plugin_session, janus_plugin_rtp *packet)
{
	...
	if (!handle || janus_flags_is_set(&handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_STOP) || janus_flags_is_set(&handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_ALERT))
		return;
...
static gboolean janus_plugin_close_pc_internal(gpointer user_data)
{
	...
	janus_refcount_increase(&ice_handle->ref);
	if (janus_flags_is_set(&ice_handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_STOP) || janus_flags_is_set(&ice_handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_ALERT))
	{
		janus_refcount_decrease(&plugin_session->ref);
		janus_refcount_decrease(&ice_handle->ref);
		return G_SOURCE_REMOVE;
...
gint main(int argc, char *argv[])
{
	...
	janus_config_category *config_database = janus_config_get_create(config, NULL, janus_config_type_category, "database");
	...
	// connect database
	if (!janus_config_get(config, config_database, janus_config_type_item, "hostname") || !janus_config_get(config, config_database, janus_config_type_item, "username") || !janus_config_get(config, config_database, janus_config_type_item, "password") || !janus_config_get(config, config_database, janus_config_type_item, "dbname"))
	{
		JANUS_LOG(LOG_FATAL, "Missing database parameter\n");
		exit(1);
	}
	mysql_handler_connect(janus_config_get(config, config_database, janus_config_type_item, "hostname")->value,
						  janus_config_get(config, config_database, janus_config_type_item, "username")->value,
						  janus_config_get(config, config_database, janus_config_type_item, "password")->value,
						  janus_config_get(config, config_database, janus_config_type_item, "dbname")->value);
	if (!mysql_is_connected())
	{
		JANUS_LOG(LOG_FATAL, "Could not connect to the databse\n");
		exit(1);
	}
	JANUS_LOG(LOG_INFO, "Connected to the database\n");
		...
		else
		{
			...
			if (!g_strcmp0(janus_plugin->get_package(), "janus.plugin.videocall") && janus_auth_is_enabled())
			{
				...
				if (res)
				{
					...
					{
						if (!janus_auth_add_token(row[0]) || !janus_auth_allow_plugin(row[0], janus_plugin))
						{
							JANUS_LOG(LOG_FATAL, "Error adding token\n");
							exit(1);
						}
					}
